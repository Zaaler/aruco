...
public:
   ...
   */
  static vector<Quadrilateral> findSquares(
    Mat gray, double limitCosine = 0.6, int minArea = 100,
    double maxError = 0.025)
  {
    //Quads found
    ...
    for (unsigned int i = 0; i < contours.size(); i++) {
      ...
      if (approx.size() == 4 && fabs(contourArea(Mat(approx))) > minArea && isContourConvex(
          Mat(
            approx)))
      {
        float maxCosine = 0;
