...
public:
   ...
   */
  static vector<ArucoMarker> getMarkers(
    Mat frame, float limitCosine = 0.7, int
    thresholdBlockSize = 7, int minArea = 100, double maxError = 0.025)
  {
    ...
    Mat thresh;
    adaptiveThreshold(
      gray, thresh, 255, THRESH_BINARY, ADAPTIVE_THRESH_MEAN_C, thresholdBlockSize,
      0.0);
  ...
  static ArucoMarker readArucoData(Mat binary)
  {
    ...
    for (int i = 0; i < binary.cols * binary.rows; i++) {
  ...
  static void drawMarkers(Mat frame, vector<ArucoMarker> markers, Mat camera, Mat distortion)
  {
    for (int i = 0; i < (int) markers.size(); i++) {
      ...
      for (int j = 0; j < 4; j++) {
        line(
          frame, markers[i].projected[j], markers[i].projected[(j + 1) % 4], Scalar(255, 0, 255),
          2);
      ...
      solvePnP(
        markers[i].info.world, markers[i].projected, camera, distortion, rotation, position,
        false, ITERATIVE);
                                #else
      solvePnP(
        markers[i].info.world, markers[i].projected, camera, distortion, rotation, position,
      ...
      //Draw number
      putText(
        frame, to_string(markers[i].id), center, FONT_HERSHEY_SIMPLEX, 0.5, Scalar(
          0, 255,
          255), 1);
    }
  }
  ...
  static void drawOrigin(
    Mat frame, vector<ArucoMarker> markers, Mat camera, Mat distortion, float
    size = 1)
  {
    if (markers.size() == 0) {
      return;
    ...
    for (unsigned int i = 0; i < markers.size(); i++) {
      ...
      for (unsigned int j = 0; j < 4; j++) {
        line(
          frame, markers[i].projected[j], markers[i].projected[(j + 1) % 4], Scalar(0, 150, 0),
          2);
      }
    }
