...
public:
  ...
  float area()
  {
    return abs(
      ...
      points[0].y) + points[2].x * (points[0].y - points[1].y)) / 2.0;
  }
  ...
  {
    return containsPoint(t.points[0]) || containsPoint(t.points[1]) || containsPoint(t.points[2]) ||
           t.containsPoint(points[0]) || t.containsPoint(points[1]) || t.containsPoint(points[2]);
  }
